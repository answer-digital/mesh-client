...
class MockMeshApplication:
    ...
    def inbox(self, environ, start_response):
        ...
        if request_method == "GET":
            if message_id:
                ...
                return _ok(
                    "application/json", [json.dumps(messages).encode("UTF-8")], start_response
                )
        else:
    ...
    def outbox(self, environ, start_response):
        ...
        return _ok(
            "application/json", [json.dumps({"messageID": msg_id}).encode("UTF-8")], start_response
        )
    ...
    def upload_chunk(self, chunk_msg):
        def handle(environ, start_response):
            ...
            with closing(stream_from_wsgi_environ(environ)) as stream:
                data = stream.read()
            chunks[chunk_num] = (
                zlib.compress(data) if not environ.get("HTTP_CONTENT_ENCODING", None) else data
            )
    ...
    def tracking(self, environ, start_response):
        ...
        for message in self.messages.values():
            if message.get("headers", {}).get("Mex-LocalID") == tracking_id:
                ...
                return _ok(
                    "application/json", [json.dumps(response).encode("UTF-8")], start_response
                )
        else:
            return _not_found(environ, start_response)
...
def main():
    ...
    server = make_server("", 8000, MockMeshApplication(MOCK_SHARED_KEY), server_class=SSLWSGIServer)
